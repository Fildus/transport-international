...
class CachingController extends AbstractController
{
    ...
    public function caching(Request $request): Response
    {
        ...
        if ($request->isXmlHttpRequest() && (int)$request->get('i') >= 0) {
            $output = shell_exec('cd /home/david/www/PRO/transport-international/ && php bin/console CODB ' . (int)$request->get('i'));
            return new JsonResponse(['data' => (int)$request->get('i')]);
        } elseif ($request->get('t') === 'reload') {
            $output = shell_exec('cd /home/david/www/PRO/transport-international/ && php bin/console doctrine:database:drop --force && php bin/console doctrine:database:create && php bin/console doctrine:schema:update --force && php bin/console CODB 1000');
